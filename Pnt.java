...
public class Pnt {
    ...
    public Pnt (Cell s ,double... coords) {
        // Copying is done here to ensure that Pnt's coords cannot be altered.
        // This is necessary because the double... notation actually creates a
        // constructor with double[] as its argument.
        coordinates = new double[coords.length];
    ...
    public Pnt midP(Pnt p)
    {
    	int len = dimCheck(p);
    	double[] coords = new double[len];
    	for(int i=0; i<len;i++)
    		coords[i] = (this.coordinates[i] + p.coordinates[i])/2;
    	return new Pnt(coords);
    ...
    /* Test if point is within boundary*/
    public boolean boundary(Pnt[] simplex)
    {
    	//System.out.println(this.coordinates[0]+"\t"+this.coordinates[1]);
    	if(this.coordinates[0]>simplex[0].coord(0)&&this.coordinates[0]<simplex[2].coord(0))
    	{
    		
    		if(this.coordinates[1]>simplex[0].coord(1)&&this.coordinates[1]<simplex[2].coord(1))
    				return true;        					    	
    	}
    	//System.out.println("Outside boundary"+this.coordinates[0]+"\t"+this.coordinates[1]);
		return false;
   ...
   Pnt Intersection(Edge l1, Edge l2, int i)
    {
    	Pnt A = l1.p1;
    	Pnt B = l1.p2;
    	Pnt C = l2.p1;
    	Pnt D = l2.p2;
        Line2D obline= new Line2D.Double(A.coord(0), A.coord(1), B.coord(0), B.coord(1)); 
	   if(obline.linesIntersect(A.coord(0),A.coord(1),B.coord(0),B.coord(1),C.coord(0),C.coord(1),D.coord(0),D.coord(1)))
	   {
		   double x,y;
		   
	    	double slope = ((A.coord(1)-B.coord(1))/(A.coord(0)-B.coord(0)));
	    	if(i==0)
	    	{
	    		x=(A.coord(0)-(A.coord(1)/slope));
	    		return new Pnt(x,0);
	    	}
	    	else if(i==1)
	    	{	x=2000;
    			y=((x-A.coord(0))*slope)+A.coord(1);
    			return new Pnt(x,y);
	    	}
	    	else if(i==2)
	    	{
	    		y=1000;
	    		x=((y-A.coord(1))/slope)+A.coord(0);
	    		return new Pnt(x,y);
	    	}
	    	else
	    	{
	    		y=(A.coord(1)-(A.coord(0)*slope));
	    		return new Pnt(0,y);
	    	}	    	
	   }
	   else
   ...
   public Pnt intersect_Pnt(Pnt A, Pnt B, Pnt C, Pnt D)
   {   	
	   Pnt p;
       Line2D obline= new Line2D.Double(A.coord(0), A.coord(1), B.coord(0), B.coord(1)); 
	   if(obline.linesIntersect(A.coord(0),A.coord(1),B.coord(0),B.coord(1),C.coord(0),C.coord(1),D.coord(0),D.coord(1)))
	   {		  
		   	...
		   	double d=(A.coord(1)-B.coord(1))*(C.coord(0)-D.coord(0)) - (A.coord(0)-B.coord(0))*(C.coord(1)-D.coord(1));
		   	/*System.out.println(A.coord(0)+"\t"+A.coord(1));
		   	System.out.println(B.coord(0)+"\t"+B.coord(1));
		   	System.out.println(C.coord(0)+"\t"+C.coord(1));
		   	System.out.println(D.coord(0)+"\t"+D.coord(1));*/
		       ...
		       double xi = ((C.coord(0)-D.coord(0))*(A.coord(0)*B.coord(1)-A.coord(1)*B.coord(0))-(A.coord(0)-B.coord(0))*(C.coord(0)*D.coord(1)-C.coord(1)*D.coord(0)))/d;
		       double yi = ((C.coord(1)-D.coord(1))*(A.coord(0)*B.coord(1)-A.coord(1)*B.coord(0))-(A.coord(1)-B.coord(1))*(C.coord(0)*D.coord(1)-C.coord(1)*D.coord(0)))/d;
		    	   //xi = (d1*A.coord(0)-d2*C.coord(0)-d3*A.coord(1)+d3*C.coord(0));
		       
		       p=  new Pnt(Math.abs(xi),Math.abs(yi));
		       /*System.out.println("Intersection"+xi+"\t"+yi);
		       System.out.println("Pnt M1: "+A.coord(0)+"\t"+A.coord(1));
		       System.out.println("Pnt M2: "+B.coord(0)+"\t"+B.coord(1));
		       System.out.println("Pnt T1: "+C.coord(0)+"\t"+C.coord(1));
		       System.out.println("Pnt T2: "+D.coord(0)+"\t"+D.coord(1));*/
		       }
   ...
   public boolean collinear(Pnt p1, Pnt p2, Pnt p3)
	{
	   ...
	   c1=p3.coord(0);
	   c2=p3.coord(1);
	   double ch = (a1*(b2 - c2) + b1*(c2 - a2) + c1*(a2 - b2));
	   int ch1= (int) (ch);
	   if (ch1 == 0)
				   return true;
		return false;
   ...
   boolean isequal(Pnt p2)
   {
	   if(this.coord(0)==p2.coord(0)&&this.coord(1)==p2.coord(1))
			   return true;
	   return false;
